...
uint32_t RtcGetCalendarTime( uint16_t *milliseconds )
{
    ...
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, /* 本进程到当前代码系统CPU花费的时间 */
        &tn); // Current seconds/sub-seconds since UNIX epoch origin
    seconds = tn.tv_sec;
    *milliseconds = tn.tv_nsec / 1000000;
...
void RtcBkupInit(void) 
{
    uint32_t seconds = 0;
    uint32_t milliseconds = 0;
    struct timespec tn = {0};
    // clock_gettime(CLOCK_MONOTONIC, // 本进程到当前代码系统CPU花费的时间
    //     &tn); // Current seconds/sub-seconds since Mcu started
    clock_gettime(CLOCK_REALTIME, /* 系统实时时间,随系统实时时间改变而改变 */
        &tn); // Current seconds/sub-seconds since UNIX epoch origin
    // clock_gettime(CLOCK_PROCESS_CPUTIME_ID, /* 系统实时时间,随系统实时时间改变而改变 */
    //     &tn); // Current seconds/sub-seconds since UNIX epoch origin
    seconds = tn.tv_sec;
    milliseconds = tn.tv_nsec / 1000000;

    int s_fd = open(RTC_BKUP_S_FILE, O_CREAT | O_WRONLY);
    if (s_fd < 0) {
        ULOG_ERR("open: %s", strerror(errno));
    }

    ftruncate(s_fd,0);
    int ret = lseek(s_fd, 0, SEEK_SET);
    if (ret < 0) {
        ULOG_ERR("lseek: %s", strerror(errno));
    }

    ret = write(s_fd, &seconds, sizeof(seconds));
    if (ret < 0) {
        ULOG_ERR("write: %s", strerror(errno));
    }
    close(s_fd);

    int ms_fd = open(RTC_BKUP_MS_FILE, O_CREAT | O_WRONLY);
    if (ms_fd < 0) {
        ULOG_ERR("open: %s", strerror(errno));
    }
    ftruncate(ms_fd,0);
    ret = lseek(ms_fd, 0, SEEK_SET);
    if (ret < 0) {
        ULOG_ERR("lseek: %s", strerror(errno));
    }
    ret = write(ms_fd, &milliseconds, sizeof(milliseconds));
    if (ret < 0) {
        ULOG_ERR("write: %s", strerror(errno));
    }
    close(ms_fd);
    ULOG_INFO("RtcBkupInit success, seconds:%u, milliseconds:%u", seconds, milliseconds);
}

// 将值存储在rtc备份寄存器中
void RtcBkupWrite(uint32_t data0, uint32_t data1)
{
    ULOG_INFO("RtcBkupWrite ...");
    ...
    ret = write(s_fd, &data0, sizeof(data0));
    if (ret < 0) {
        ULOG_ERR("write: %s", strerror(errno));
    }
    close(s_fd);
    ...
    ret = write(ms_fd, &data1, sizeof(data1));
    if (ret < 0) {
        ULOG_ERR("write: %s", strerror(errno));
    }
    close(ms_fd);
    ULOG_INFO("RtcBkupWrite, data0:%u,data1:%u", data0, data1);
}

void RtcBkupRead(uint32_t *data0, uint32_t *data1)
{
    ULOG_INFO("RtcBkupRead ...");
    int s_fd = open(RTC_BKUP_S_FILE, O_CREAT | O_RDONLY);
    if (s_fd < 0) {
        ULOG_ERR("open: %s", strerror(errno));
    }
    ...
    ret = read(s_fd, data0, sizeof(uint32_t));
    if (ret < 0) {
        ULOG_ERR("read: %s", strerror(errno));
    }
    close(s_fd);

    int ms_fd = open(RTC_BKUP_MS_FILE, O_CREAT | O_RDONLY);
    if (ms_fd < 0) {
        ULOG_ERR("open: %s", strerror(errno));
    }
    ...
    ret = read(ms_fd, data1, sizeof(uint32_t));
    if (ret < 0) {
        ULOG_ERR("read: %s", strerror(errno));
    }
    close(ms_fd);
    ULOG_INFO("RtcBkupRead, data0:%u,data1:%u", *data0, *data1);
}
