...
static modbus_t *ctx;
static struct edge_device dev = {.name = "TPEP"};
...
static void modbus_read_cb(struct uloop_timeout *timeout)
{
    ...
    rc = modbus_read_input_registers(ctx, 1, 6, regs);
    if (rc < 0) {
        DBG_ERR("read registers failed '%s'", strerror(errno));
        goto out;
    }

    DBG_INFO("read %d registers, [0]: %d, [1]: %d, [2]: %d, [3]: %d, [4]: %d, [5]: %d", rc, regs[0], regs[1], regs[2], regs[3], regs[4], regs[5]);
